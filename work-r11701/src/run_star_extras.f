...
!		no diffusion for fully convective, post-MS, and mega-old models 
         ...
         if(s% center_h1 < 1e-4 .and. s% center_he4 < s% x_ctrl(2)) then
            extras_check_model = terminate
            write(*, *) 'center_he4 < center_he4_limit'
            return
         end if
      ...
      integer function extras_finish_step(id, id_extra)
       ...
       ! set BC: change to tables after running on simple photosphere
        if (s% x_logical_ctrl(2) .and. s% model_number == 100) then
          write(*,*) '++++++++++++++++++++++++++++++++++++++++++++++++++'
          write(*,*) 'switching from simple photosphere to ', &
                      s% job% extras_cpar(1)
          write(*,*) '++++++++++++++++++++++++++++++++++++++++++++++++++'
          s% which_atm_option = s% job% extras_cpar(1)
        endif
        
        ! save models of pre-sdB stars
        if (s% center_h1 < 1e-4 .and. s% x_logical_ctrl(1) .and. &
         mod(s% model_number, s% x_integer_ctrl(1)) == 0 .and. &
         safe_log10_cr(s% Teff) <  s% x_ctrl(1) .and. &
         safe_log10_cr(s% L_phot) >  s% x_ctrl(3)) then
          write(file_number, '(i5.5)') s% model_number
          call star_write_model(id, 'LOGS/model_' // file_number // '.mod', ierr)
          ! write(*,2) 'model' // trim(s% model_profile_filename) // '.mod', nz
